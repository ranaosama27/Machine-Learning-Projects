...
#one hote enconding for data train and test 
Class_train = []
Class_test  = []
...
for i in y_Train:
  class_verctor =[]
  for j in range(0,62): # 62 classes 
      if (i==j):
        class_verctor.append(1)
      else:
        class_verctor.append(0)
  Class_train.append(class_verctor)
...
for i in y_Test:
  ...
  for j in range(0,62): # 62 classes 
      if (i==j):
        ...
        class_verctor.append(0)
  Class_test.append(class_verctor)
...
x = tf.placeholder(tf.float32,shape=[None,784]) # 28 * 28 *1 = 784
y_true = tf.placeholder(tf.float32,shape=[None,62])

x_image = tf.reshape(x,[-1,28,28,1])

convo_1 = convolutional_layer(x_image,shape=[6,6,1,32]) #  32 filter size = 6 pixle  , 1chanle 
...
with tf.Session() as sess:
    ...
    for i in range(steps):
        ...
        batch_x , batch_y = x_Train[i:i+50],Class_train[i:i+50]
        ...
        if i%100 == 0:
            ...
            print(sess.run(acc,feed_dict={x:x_Test,y_true:Class_test,hold_prob:1.0}))
            print('\n')
